...
Fn.get_H2H_damage = function(draw)
  local str_mod = types.Player.stats.attributes.strength(self).base / getSetting('HandToHand_Strength')
...
local unarmored_hit_decay = time.runRepeatedly(function()
    if Dt.counters.unarmored(0) > 0.01 then
      Dt.counters.unarmored(- math.max(Dt.counters.unarmored(0)/getSetting('Unarmored_Decay_Time'), 2/(getSetting('Unarmored_Decay_Time'))))
...
local acrobatics_jump_decay = time.runRepeatedly(function()
    if Dt.counters.acrobatics(0) > 0.01 then
      Dt.counters.acrobatics(- math.max(Dt.counters.acrobatics(0)/getSetting('Acrobatics_Decay_Time'), 2/(getSetting('Acrobatics_Decay_Time'))))
...
local athletics_run_decay  = time.runRepeatedly(function()
    if Dt.counters.athletics(0) > 0.01 then
      Dt.counters.athletics(- math.max(Dt.counters.athletics(0) - getSetting('Athletics_Decay_Time'), 1))
    else Dt.counters.athletics(-Dt.counters.athletics(0))
    end
    if Dt.counters.athletics_debug(0) > 9.01 then
      if getSetting(SUS_DEBUG) then print('SUS [Athletics] Decay Timer: '.. string.format('%.0f', Dt.counters.athletics(0))) end
    end
  end, time.second)
...
-----------------------------------------------------------------------------------------------------------
  for _, _skillid in ipairs(Dt.scaler_groups.ARMOR) do
    Dt.scalers:new{
      name = _skillid,
      func = function(_, xp)
        if not getSetting('toggle_armor') then return xp end
        if not getSetting(_skillid) then return xp end
        ...
        if Fn.has_effect('disintegratearmor') then
          if getSetting(SUS_DEBUG) then print('SUS - Disintegrate Armor in effect, returning vanilla XP') end
          return xp
        end
        ...
        local multiplier = damage/getSetting('Armor_Damage_To_XP ') * 2*Dt.GMST.iBaseArmorSkill / (Dt.GMST.iBaseArmorSkill + skill)
        xp = xp * multiplier
        ...
        if getSetting(SUS_DEBUG) then print('SUS [Armor] Skill Uses: '.. string.format('%.2f', multiplier)..' | Skill Progress: '..percentify(xp)..' | Damage Received: '.. string.format('%.2f', damage)) end
...
-----------------------------------------------------------------------------------------------------------
  Dt.scalers:new{ name = 'block',
    func = function(_, xp)
      if not getSetting('toggle_armor') then return xp end
      if not getSetting('block') then return xp end
      ...
      local multiplier = damage/getSetting('Block_Damage_To_XP ') * (Dt.GMST.iBlockMaxChance+Dt.GMST.iBlockMinChance) / (2*Dt.GMST.iBlockMinChance + skill)
      xp = xp * multiplier

      if getSetting(SUS_DEBUG) then print('SUS [Block] Skill Uses: '.. string.format('%.2f', multiplier)..' | Skill Progress: '..percentify(xp)..' | Damage Received: '.. string.format('%.2f', damage)) end
...
-----------------------------------------------------------------------------------------------------------
  Dt.scalers:new{ name = 'unarmored', 
    func = function(_, xp)
      if not getSetting('unarmored') then return xp end
...
--       if #Fn.get_unarmored_slots() <= 3 and (race == 'argonian' or race == 'khajiit') then beast_factor = getSetting('Unarmored_Beast_Races ')/ #Fn.get_unarmored_slots() end
      local gank_factor  = (getSetting('Unarmored_Start ') - getSetting('Unarmored_Min')) / (2 * Dt.counters.unarmored(1) - 1) + getSetting('Unarmored_Min')
      ...
      local skill_factor = 100 / (35 + rating + skill + armor_weight * getSetting('Unarmored_Armor_Mult')) -- Rating is added here alongside skill, because unarmored has exponential scaling baked in.

      local multiplier = skill_factor * beast_factor * gank_factor
      local xp = xp * multiplier

      if getSetting(SUS_DEBUG) then print('SUS [Unarmored] Skill Uses: '.. string.format('%.2f', multiplier)..' | Skill Progress: '..percentify(xp)..' | Hit Counter: '..string.format('%.2f', Dt.counters.unarmored(0))) end
...
-----------------------------------------------------------------------------------------------------------
  for _, _skillid in ipairs(Dt.scaler_groups.SPELL) do
    Dt.scalers:new{ name = _skillid, 
      func = function(_, xp)
        if not getSetting('toggle_magic') then return xp end
        if not getSetting(_skillid) then return xp end

        local spell = Dt.pc.spell
        if not spell then 
          if getSetting(SUS_DEBUG) then print('SUS - Held Spell not found, returning vanilla XP') end
          return xp
        end
        local max_mp   = types.Player.stats.dynamic.magicka(self).base
        local mp_factor  = 0.01*max_mp
        local multiplier = spell.cost/getSetting('Magicka_to_XP') * 4.8/(4 + math.max(0, mp_factor - 1))
        xp = xp * multiplier

        if getSetting(SUS_DEBUG) then print('SUS [Magic] Skill Uses: '.. string.format('%.2f', multiplier)..' | Skill Progress: '..percentify(xp)..' | Spell Cost: '.. string.format('%.2f', spell.cost)) end

        if not getSetting('toggle_refund') then return xp end

        local armor_weight = 0
        local armor    = Fn.get_equipped_armor()
        for _, _obj in ipairs(armor) do armor_weight = armor_weight + types.Armor.record(_obj).weight end
        local armor_offset = armor_weight * getSetting('MP_Refund_Armor_Mult')
        local cost_factor  = max_mp / (max_mp + spell.cost)
        local skill    = types.Player.stats.skills[_skillid](self).base
        local skill_factor = (skill - getSetting('MP_Refund_Skill_Offset') - armor_offset) / (40 + skill)

        local refund = spell.cost * cost_factor * skill_factor * 0.01*getSetting('MP_Refund_Max_Percent')
        ...
        if refund > 0 then
          if getSetting(SUS_DEBUG) then print('SUS [Dynamic Spell Cost] Refund: '.. string.format('%.2f', refund/spell.cost*100)..'% | '.. string.format('%.2f', refund) ..' MP') end
        else
          if getSetting(SUS_DEBUG) then print('SUS [Dynamic Spell Cost] Penalty: '.. string.format('%.2f', refund/spell.cost*100)..'% | '.. string.format('%.2f', refund) ..' MP') end
        end
...
-----------------------------------------------------------------------------------------------------------
  for _, _skillid in ipairs(Dt.scaler_groups.WEAPON) do
    Dt.scalers:new{ name = _skillid, 
      func = function(_, xp)
        if not getSetting('toggle_physical') then return xp end
        if not getSetting(_skillid) then return xp end

        local skill = types.Player.stats.skills[_skillid](self).base
        local multiplier = Dt.pc.attack.damage/getSetting('Physical_Damage_to_XP') * 80/(40 + skill)
        xp = xp * multiplier
        if getSetting(SUS_DEBUG) then print('SUS [Weapon] Skill Uses: '.. string.format('%.2f', multiplier)..' | Skill Progress: '..percentify(xp)..' | Damage Dealt: '.. string.format('%.2f', Dt.pc.attack.damage)..' | Weapon Draw // Attack Charge: '..percentify(Dt.pc.attack.draw)) end
        return xp
      end
    }
  end
...
-----------------------------------------------------------------------------------------------------------
  Dt.scalers:new{ name = 'handtohand', 
    func = function(_, xp)
      if not getSetting('toggle_physical') then return xp end
      if not getSetting('handtohand') then return xp end
      ...
      local multiplier = damage/getSetting('Physical_Damage_to_XP') * 80/(40 + skill)
      xp = xp * multiplier
      if getSetting(SUS_DEBUG) then print('SUS [Hand-To-Hand] Skill Uses: '.. string.format('%.2f', multiplier)..' | Skill Progress: '..percentify(xp)..' | Damage Dealt: '.. string.format('%.2f', damage)..' | Weapon Draw // Attack Charge: '..percentify(Dt.pc.attack.draw)) end
      return xp
    end
  }
  -- ACROBATICS
-----------------------------------------------------------------------------------------------------------
  Dt.scalers:new{ name = 'acrobatics', 
    func = function(useType, xp)
      if not getSetting('acrobatics') then return xp end

      local encumbered_mult = getSetting('Acrobatics_Encumbrance_Min') + (getSetting('Acrobatics_Encumbrance_Max') - getSetting('Acrobatics_Encumbrance_Min'))
      ...
      if useType == skp.SKILL_USE_TYPES.Acrobatics_Jump then
        recursive_mult = (getSetting('Acrobatics_Start')) / (1 + (Dt.counters.acrobatics(1) -1)/2)
      ...
      xp = xp * multiplier
      if getSetting(SUS_DEBUG) then print('SUS [Acrobatics] Skill Uses: '.. string.format('%.2f', multiplier)..' | Skill Progress: '..percentify(xp)..' | Jump Mult: '.. string.format('%.2f', recursive_mult)) end
      return xp
    end
  }
  -- ATHLETICS
-----------------------------------------------------------------------------------------------------------
  Dt.scalers:new{ name = 'athletics', 
    func = function(useType, xp)
      if not getSetting('athletics') then return xp end
      ...
      local move_factor = 1
      if movement < 150 then move_factor = getSetting('Athletics_No_Move_Penalty') end
      Dt.pc.position = position
      local encumbered_mult = getSetting('Athletics_Encumbrance_Min') + (getSetting('Athletics_Encumbrance_Max') - getSetting('Athletics_Encumbrance_Min'))
                  * types.Actor.getEncumbrance(self) / (Dt.GMST.fEncumbranceStrMult * types.Player.stats.attributes.strength(self).base)
      local recursive_mult  = (getSetting('Athletics_Start')) + (getSetting('Athletics_Marathon') - (getSetting('Athletics_Start'))) * (Dt.counters.athletics(2) -2)/getSetting('Athletics_Decay_Time')
      ...
      if printcounter > 5.01 then
        if getSetting(SUS_DEBUG) then print('SUS [Athletics] Skill Uses: '.. string.format('%.2f', multiplier)..' | Skill Progress: '..percentify(xp * printcounter)..' | Marathon Mult: '.. string.format('%.2f', recursive_mult)) end
...
-----------------------------------------------------------------------------------------------------------
  Dt.scalers:new{ name = 'security', 
    func = function(useType, xp)
      if not getSetting('security') then return xp end
      ...
      if (useType == skp.SKILL_USE_TYPES.Security_PickLock) and Dt.pc.security_target.islocked then
        multiplier = target.level/getSetting('Security_Lock_Points_To_XP')
      ...
      elseif (useType == skp.SKILL_USE_TYPES.Security_DisarmTrap) and Dt.pc.security_target.trap then
        multiplier = target.trap.cost/getSetting('Security_Trap_Points_To_XP')
        security_print = 'Trap Level: '..target.trap.cost
      end

      xp = xp * multiplier
      if getSetting(SUS_DEBUG) then print('SUS [Security] Skill Uses: '.. string.format('%.2f', multiplier)..' | Skill Progress: '..percentify(xp)..' | '..security_print) end
      return xp
    end
  }

  if getSetting(SUS_DEBUG) then print('SUS: Scalering Commenced') end
end
